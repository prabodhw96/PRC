...
st.markdown("**Daily Weather Information reported by NOAA (takes more time to download)**")
...
if st.button('Weather data'):
    tmp_download_link = download_link(weather_updated, 'weather_updated.csv', 'Click here to download')
    st.markdown(tmp_download_link, unsafe_allow_html=True)